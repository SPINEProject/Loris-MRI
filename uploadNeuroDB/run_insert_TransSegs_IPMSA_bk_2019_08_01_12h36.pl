...
run_insert_segmentation.pl -- a script that inserts a segmentation into LORIS.


=head1 SYNOPSIS

C<perl tools/run_insert_segmentation.pl [options]>
...
C<-visitLabel>: Visit label of the candidate in LORIS C<visitLabel>
...
C<-acquisitionLabel>:  C<acquisitionLabel>
...
This script inserts a segmentaiton into LORIS attached to a file. The file is
...
## Link the fileregister_file_t2_lesions_aligned_with_icbm_template($FileID);
register_file_t2_lesions_aligned_with_icbm_template($FileID);
...
sub register_file_t2_lesions_aligned_with_icbm_template {
    my ($FileID) = @_;

    my $register_cmd = "register_processed_data.pl "
                       . " -profile $profile "
                       . " -sourcePipeline NeuroRx_name_to_be_confirmed "
                       . " -tool 'NeuroRx_name_to_be_confirmed' "
                       . " -pipelineDate $today "
                       . " -coordinateSpace ICBM "
                       . " -outputType ct2f_ISPC-stx152lsq6 "
		       . " -inputFileIDs $FileID "
                       . " -sourceFileID $FileID"
                       . " -scanType $fileType"
                       . " -file $filepath";
